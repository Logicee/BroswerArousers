<!DOCTYPE html>

<html>
    <head>
        <meta charset="UTF-8">
        <title>Tournament Standings</title>

      
        <script src="tournamentStandings.js"></script>  
       <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous"/>
        <link rel="stylesheet" href="bowling1.css"/>
        <link rel="stylesheet" href="tournamentStandings.css"/>
    </head>
    <body>
      <div class=" justify-content-center row mt-2">
            <div class="col-10">
                <h1>Bowling Tournament Standings</h1>
            </div>
            <div class="col-1-auto">
                <form action="index.php">
                    <button class="mt-4 mr-3">Log Out</button>  
                </form>
                
                
            </div>
            <div class="col-1-auto">
                <img id="logo" src="bowling.png" class="img-thumbnail rounded float-right" alt="Bowling App logo"/>
                
            </div>
        </div>
               <div class=" justify-content-center row mt-2 mb-4 border-bottom">
            <div class="col-12">
                <form action="disha.php"><button class="mb-2">Back</button></form>
            </div>
         
        </div>
        <div class="brackets" id="brackets">
            <div class="wrappingdiv">


                <div class="r1" id="round1">
                    <label>Seeded</label>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1" >&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 

                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <label>Random</label>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>



                </div>


                <div class="r2" id="round2">
                    <label>&nbsp;</label>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    
                    <div >
                        
                        <div class="bracket pusher"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 
  

                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>




                </div>    


                <div class="r3" id="round3">
                    <label>&nbsp;</label>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    
                    <div>
                        <div class="bracket pusher2"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>

                </div>  

                <div class="r4" id="round4">
                    <label>&nbsp;</label>
                    <div>
                        <div class="bracket"> 


                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>
                  
                    <div>
                        <div class="bracket pusher3"> 
                         

                            <span class="team1">&nbsp;</span> 
                            <span class="team2">&nbsp;</span>
                        </div>
                    </div>



                </div>  

                <div class="r5 " id="final">
                    <label>&nbsp;</label>
                    <div>
                        <div class="bracket pusher4"> 


                            <span class="team1">&nbsp;</span> 
                            
                            <span class="team2 ">&nbsp;</span>
                        </div>
                    </div>




                </div>  

                <div class="r6" >
                    <label>&nbsp;</label>
                    <div class="final">
                        <div class="bracket"> 


                            <span class="winner">&nbsp;</span> 

                        </div>
                    </div>




                </div>  
            </div>


        </div>

    </body>
</html>
